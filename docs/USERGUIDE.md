...
* **JamesDSP (O2P only)** - Toggle enabling or disabling loading the JamesDSP library via the "Run script as Root" functionality. JamesDSP Manager app is required and presented as an install option on this screen.
...
Running on a rooted device will make some tweaks available and allow certain existing tweaks to be applied boot instead of runtime. This is done by dynamically generating a Magisk module with the relevant options, which is then loaded on startup.
...
* **JamesDSP** - Install JamesDSP via a Magisk module instead of the typical method. This avoids the need for O2P Tweaks to run at startup, and is the recommended method for using JamesDSP on a rooted device.
